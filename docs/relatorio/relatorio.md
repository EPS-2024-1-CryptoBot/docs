...
| Kevin Luis Apolinario Batista | Criação do relatório contínuo, desenvolvimento da visualização de extrato e histórico de valores de criptomoedas, configuração completa do Backend e por integrar os microsserviços da nossa equipe e de outras equipes (Stock Compass e Pitch It) com ele, configuração do Frontend do projeto e responsável por algumas integrações com do front com o gateway e criação de interfaces, negociação da máquina na Atena Solutions para ter uma máquina disponível para o gateway e fora da AWS  | Excelente |
